...
__all__ = ['DPDETR_HORIZON']
# My multispectral DETR


@register
class DPDETR_HORIZON(BaseArch):
    ...
    def __init__(self,
        ...
        super(DPDETR_HORIZON, self).__init__()
    ...
    def _forward(self):
        ...
        if self.training:
            ...
            if 'infer' not in self.inputs.keys():
                #normal
                ...
                #paired infer
                preds = self.detr_head(out_transformer, None, infer=True)
                ...
                if self.exclude_post_process:
                    ...
                    bbox_vis,bbox_ir, bbox_num, mask = self.post_process(
                        preds, self.inputs['im_shape'], self.inputs['scale_factor'],
                        paddle.shape(self.inputs['vis_image'])[2:], infer=True)
