...
__all__ = ['DETRLoss', 'DINOLoss','DETRLoss_obb_Decoupled_Position','DINOLoss_obb_Decoupled_Position',
           'DETRLoss_Decoupled_Position', 'DINOLoss_Decoupled_Position']


@register
class DETRLoss_obb_Decoupled_Position(nn.Layer):
    __shared__ = ['num_classes', 'use_focal_loss']
    __inject__ = ['matcher']

    def __init__(self,
                 num_classes=80,
                 matcher='HungarianMatcher_obb_Decoupled_Position',
                 loss_coeff={
                     'class': 1,
                     'bbox': 5,
                     'giou': 2,
        ...
        """
        super(DETRLoss_obb_Decoupled_Position, self).__init__()
...
@register
class DINOLoss_obb_Decoupled_Position(DETRLoss_obb_Decoupled_Position):
    def forward(self,
                boxes_vis,
                boxes_ir,
                logits,
        ...
        num_gts = self._get_num_gts(gt_class)
        num_quires = 300
        total_loss = super(DINOLoss_obb_Decoupled_Position, self).forward(
        ...
        if dn_meta is not None:
            ...
            dn_loss = super(DINOLoss_obb_Decoupled_Position, self).forward(
                dn_out_bboxes_vis,
                dn_out_bboxes_ir,
                dn_out_logits,
